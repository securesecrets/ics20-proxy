...
pub fn execute_receive(
    ...
    let api = deps.api;
    execute_transfer(deps, env, info, msg, amount, api.addr_validate(&wrapper.sender)?)
}
...
pub fn execute_transfer(
    ...
    let code_hash = match msg.token_code_hash {
        None => {
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::ListChannels {} => Err(StdError::generic_err("Not Implemented")),//to_binary(&query_list(deps)?),
        QueryMsg::Channel { id } => Err(StdError::generic_err("Not Implemented")),//to_binary(&query_channel(deps, id)?),
