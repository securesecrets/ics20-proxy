...
pub mod v2 {
    ...
    use shade_protocol::{
        c_std::{
            to_binary, Addr, DepsMut, Env, Order, StdResult, WasmQuery,
        },
        snip20::{self, BalanceResponse},
    };
    ...
    fn update_denom(
        ...
        let balance = match Amount::from_parts(denom.clone(), state.outstanding) {
            ...
            Amount::Snip20(coin) => {
                ...
                // `Snip20Contract(contract.clone()).balance(&deps.querier, contract)?`
